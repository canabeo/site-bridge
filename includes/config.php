...
class SB_Config {
	...
	/** Option name used when secret is not defined as a constant. */
	const OPT_SECRET = 'sb_secret';

	/**
	 * Current active secret.
	 *
	 * Priority order:
	 *   1. `SITE_BRIDGE_SECRET` constant in wp-config.php  (recommended — survives wp-admin compromise)
	 *   2. `sb_secret` option in wp_options              (fallback for installs without wp-config access,
	 *                                                       generated at plugin activation; see SB_Installer)
	 *
	 * @return string|null  null if neither source has a long-enough secret.
	 */
	public static function get_secret() {
		...
		// Fallback: secret stored as an option (generated on activation when no wp-config constant).
		$opt = get_option( self::OPT_SECRET );
		if ( is_string( $opt ) && strlen( $opt ) >= self::MIN_SECRET_BYTES ) {
			return $opt;
		}
		return null;
	}

	/** True when the active secret is the autogenerated option (i.e. user did not set the constant). */
	public static function secret_is_option_fallback() {
		if ( defined( 'SITE_BRIDGE_SECRET' )
		  && strlen( (string) SITE_BRIDGE_SECRET ) >= self::MIN_SECRET_BYTES ) {
			return false;
		}
		$opt = get_option( self::OPT_SECRET );
		return is_string( $opt ) && strlen( $opt ) >= self::MIN_SECRET_BYTES;
	}
